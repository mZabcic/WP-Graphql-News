...
class Media {
  ...
  public function add_custom_image_sizes() {
    ...
    // From facebook
    add_image_size('home-article',600,9999,false);
    add_image_size('slick', 254 );
    add_image_size('listing-grid', 800, 600, true );
  }
      ...
      try {
        ...
        if ( file_exists( $path ) ) {
          ...
          if ( ! $this->general_helper->is_valid_xml( $svg_content ) ) {
            new \WP_Error( sprintf( esc_html__( 'Error: File invalid: %s', 'wpbit4bytes' ), $path ) );
            return false;
          }
        ...
        new \WP_Error( sprintf( esc_html__( 'Error: %s', 'wpbit4bytes' ), $e ) );
      }
    }

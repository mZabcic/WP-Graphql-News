...
wp core install --url={temporis2.test} --title={THEMENAME} --admin_user={ADMINUSER} --admin_email={ADMINMAIL}
...
```bash
wpcs wp-content/themes/wpbit4bytes
```

Autofix theme for minor violations:

```bash
wpcbf wp-content/themes/wpbit4bytes
```
